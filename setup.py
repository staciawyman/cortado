#!/usr/bin/env python
"""Description:
Setup script for cortado -- Software pipeline for the analysis of CRISPR-Cas9 genome editing outcomes from deep sequencing data
@author:  Stacia Wyman from Luca Pinello's CRISPResso
@contact: staciawyman@berkeley.edu
"""
...
#TO INSTALL cortado DEPENDECIENS IN A CUSTOM LOCATION SET THE ENV VARIABLE: CORTADO_DEPENDENCIES_FOLDER
if os.environ.get('CORTADO_DEPENDENCIES_FOLDER'):
	INSTALLATION_PATH=os.environ.get('CORTADO_DEPENDENCIES_FOLDER')
else:
	INSTALLATION_PATH='%s/cortado_dependencies' % os.environ['HOME']
...
def main():
	...
	setup(
    	  ...
    	  packages = ["cortado"],
    	  package_dir={'cortado': 'cortado'},
          package_data={'cortado': ['data/*']},
    	  entry_points = {
        	"console_scripts": ['cortado = cortadoCORE:main']
           },
          description="Software pipeline for the analysis of CRISPR-Cas9 genome editing outcomes from deep sequencing data",
          author='Stacia Wyman from Luca Pinellos CRISPResso',
          author_email='staciawyman@berkeley.edu',
          url='http://github.com/staciawyman/cortado',
...
def check_flash():
	if which('flash'):
		...
		sys.stdout.write('\ncortado requires a recent version FLASh from: http://ccb.jhu.edu/software/FLASH/')
...
def check_needle():
	if which('needle'):
		...
		sys.stdout.write ('\ncortado requires a recent version needle from the EMBOSS suite(>=6): http://emboss.sourceforge.net/download/#Stable/')
...
def install_dependencies():
	...
	if CURRENT_PLATFORM not in  ['Linux','Darwin'] and platform.architecture()!='64bit':
		sys.stdout.write('Sorry your platform is not supported\n cortado is supported only on 64bit versions of Linux or OSX ')
		sys.exit(1)
	...
	if dependencies_already_installed:
		...
		#ADD cortado  dependencies to PATH
		home = expanduser("~")
		...
		sys.stdout.write ('\n\nINSTALLATION COMPLETED, open a NEW terminal to run cortado'    )
