...
my $threads = 30;
my $window_size = 32;   
my $cortado_path = `which cortado_htt.py`;
chomp($cortado_path);
...
while (<>) {
    ...
    if ($donorseq eq "-") { # Run as just NHEJ
        print "/usr/bin/python $cortado_path -r1 $fastq_path/$samp\_trim.fastq.gz  -o output -n $name -a $refseq -g $guideseq --trim_sequences  --keep_intermediate  --min_identity_score 65 --window_around_sgrna $window_size  --min_frequency_alleles_around_cut_to_plot 0.1 --max_rows_alleles_around_cut_to_plot 250 ";
